...
def manhattan_distance(
    position_a: Tuple[int, int, int], position_b: Tuple[int, int, int]
) -> int:
    (a_x, a_y, a_z) = position_a
    (b_x, b_y, b_z) = position_b
    return abs(b_x - a_x) + abs(b_y - a_y) + abs(b_z - a_z)
...
def solve() -> int:
    ...
    scanner_positions: Set[Tuple[int, int, int]] = set([(0, 0, 0)])
    while priority_queue:
        ...
        scanner_position: Tuple[int, int, int] = (0, 0, 0)
        for i, j in zip(path, path[1:]):
            ...
            scanner_position = a.transform_position(
                scanner_position, signs, beacon_relative, indices
            )
        scanner_positions.add(scanner_position)
        solved_count.add(idx)

    max_manhattan_distance = -maxsize
    for (position_a, position_b) in combinations(scanner_positions, 2):
        max_manhattan_distance = max(
            manhattan_distance(position_a, position_b), max_manhattan_distance
        )
