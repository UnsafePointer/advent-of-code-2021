...
def solve() -> int:
    ...
    useful_probes: Set[Probe] = set()

    for x_velocity in range(-max_value, max_value):
        for y_velocity in range(-max_value, max_value):
            probe = try_probe((x_velocity, y_velocity), target_range)
            if probe:
                useful_probes.add(probe)

    max_height = -maxsize
    for probe in useful_probes:
        max_height = max(max_height, probe.highest_vertical_position)
    return max_height
