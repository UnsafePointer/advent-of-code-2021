...
def calculate_energy_cost(_type: str, x1: int, y1: int, x2: int, y2: int) -> int:
    ...
    if is_vertial_location_in_hallway(y1):
        ...
        return (abs(2 - y1) + abs(2 - y2) + abs(x1 - x2)) * energy_cost[_type]
...
def can_enter_own_room(
    ...
    if ampiphod_in_room >= 2:  # Room is full
        return False
    if (
        ampiphod_in_room == 1 and rooms_state[target_room][_type] != 1
